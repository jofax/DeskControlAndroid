...
fun BluetoothDevice.smartpodsBleDevice(adapter: BluetoothAdapter): BluetoothDevice? {
    val mBleUUID = Constants.BLE_UUID
    if (adapter != null) {
        return adapter.getRemoteDevice(mBleUUID)
    }
    return null
}
...
fun BluetoothGattDescriptor.isCccd() =
    uuid.toString().toUpperCase(Locale.US) == Constants.CCC_DESCRIPTOR_UUID.toUpperCase(Locale.US)
